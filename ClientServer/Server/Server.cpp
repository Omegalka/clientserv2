...
int main()
{
    ...
    const char* commands[5] = { "котра година?", "привiт", "погода", "новини", "до побачення" };
    const char* responses[5] = { "14:44", "Привiт", "Сьогодні температура -50°C", "Останні новини: ...", "До побачення" };
    ...
    do {
        ...
        if (iResult > 0) {
            ...
            double receivedNumber = atof(message);
            receivedNumber += 1;
            string response = to_string(receivedNumber);

            cout << "сервер відповідає: " << response << "\n";
